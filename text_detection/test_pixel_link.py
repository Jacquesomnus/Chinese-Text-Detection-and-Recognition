...
tf.flags.DEFINE_integer('num_preprocessing_threads', 4, 'The number of threads used to create the batches.')
tf.flags.DEFINE_bool('preprocessing_use_rotation', False, 'Whether to use rotation for data augmentation')

# =========================================================================== #
# Dataset Flags.
# =========================================================================== #
tf.flags.DEFINE_string('dataset_name', 'icdar2015', 'The name of the dataset to load.')
tf.flags.DEFINE_string('dataset_split_name', 'test', 'The name of the train/test split.')
tf.flags.DEFINE_string('dataset_dir', util.io.get_absolute_path('~/dataset/ICDAR2015/Challenge4/ch4_test_images'),
                       'The directory where the dataset files are stored.')
tf.flags.DEFINE_integer('eval_image_width', 1280, 'Train image size')
tf.flags.DEFINE_integer('eval_image_height', 768, 'Train image size')
...
def config_initialization():
    ...
    config.init_config(image_shape, batch_size=1, num_gpus=1, pixel_conf_threshold=0.8, link_conf_threshold=0.8)

    util.proc.set_proc_name('test_pixel_link_on' + '_' + FLAGS.dataset_name)
    

def to_txt(txt_path, image_name, image_data, pixel_pos_scores, link_pos_scores):
...
def test():
    ...
    with tf.Session(config=sess_config) as sess:
        ...
        for iter, image_name in enumerate(image_names):
            ...
            pixel_pos_scores, link_pos_scores = sess.run([net.pixel_pos_scores, net.link_pos_scores],
                                                         feed_dict={image: image_data})
               
            print '%d/%d: %s' % (iter + 1, len(image_names), image_name)
            to_txt(txt_path, image_name, image_data, pixel_pos_scores, link_pos_scores)
