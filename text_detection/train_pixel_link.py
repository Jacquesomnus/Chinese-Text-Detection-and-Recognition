...
# =========================================================================== #
tf.flags.DEFINE_string('train_dir', None, 'the path to store checkpoints and eventfiles for summaries')

tf.flags.DEFINE_string('checkpoint_path', None, 'the path of pretrained model to be used. '
                                                'If there are checkpoints in train_dir, this config will be ignored.')

tf.flags.DEFINE_integer('batch_size', None, 'The number of samples in each batch.')
tf.flags.DEFINE_integer('num_gpus', 1, 'The number of gpus can be used.')
tf.flags.DEFINE_integer('max_number_of_steps', 1000000, 'The maximum number of training steps.')
tf.flags.DEFINE_integer('log_every_n_steps', 1, 'log frequency')
tf.flags.DEFINE_bool("ignore_missing_vars", False, '')
tf.flags.DEFINE_string('checkpoint_exclude_scopes', None, 'checkpoint_exclude_scopes')

# =========================================================================== #
# Optimizer configs.
# =========================================================================== #
tf.flags.DEFINE_float('learning_rate', 0.001, 'learning rate.')
tf.flags.DEFINE_float('momentum', 0.9, 'The momentum for the MomentumOptimizer')
tf.flags.DEFINE_float('weight_decay', 0.0001, 'The weight decay on the model weights.')
tf.flags.DEFINE_float('moving_average_decay', 0.9999, 'The decay rate of ExponentionalMovingAverage')

# =========================================================================== #
# I/O and preprocessing Flags.
# =========================================================================== #
tf.flags.DEFINE_integer('num_readers', 1, 'The number of parallel readers that read data from the dataset.')
tf.flags.DEFINE_integer('num_preprocessing_threads', 24, 'The number of threads used to create the batches.')

# =========================================================================== #
# Dataset Flags.
# =========================================================================== #
tf.flags.DEFINE_string('dataset_name', None, 'The name of the dataset to load.')
tf.flags.DEFINE_string('dataset_split_name', 'train', 'The name of the train/test split.')
tf.flags.DEFINE_string('dataset_dir', None, 'The directory where the dataset files are stored.')
tf.flags.DEFINE_integer('train_image_width', 512, 'Train image size')
tf.flags.DEFINE_integer('train_image_height', 512, 'Train image size')
...
def config_initialization():
    ...
    util.init_logger(log_file='log_train_pixel_link_%d_%d.log' % image_shape, log_path=FLAGS.train_dir, stdout=False,
                     mode='a')
...
def create_dataset_batch_queue(dataset):
    ...
    with tf.device('/cpu:0'):
        with tf.name_scope(FLAGS.dataset_name + '_data_provider'):
            provider = slim.dataset_data_provider.DatasetDataProvider(dataset, num_readers=FLAGS.num_readers,
                                                                      common_queue_capacity=1000*config.batch_size,
                                                                      common_queue_min=700*config.batch_size,
                                                                      shuffle=True)
        # Get for SSD network: image, labels, bboxes.
        [image, glabel, gbboxes, x1, x2, x3, x4, y1, y2, y3, y4] = provider.get(['image', 'object/label', 'object/bbox',
        ...
        image, glabel, gbboxes, gxs, gys = ssd_vgg_preprocessing.preprocess_image(image, glabel, gbboxes, gxs, gys,
        ...
        with tf.name_scope(FLAGS.dataset_name + '_batch'):
            b_image, b_pixel_cls_label, b_pixel_cls_weight, b_pixel_link_label, b_pixel_link_weight = \
                tf.train.batch([image, pixel_cls_label, pixel_cls_weight, pixel_link_label, pixel_link_weight],
                               batch_size=config.batch_size_per_gpu, num_threads=FLAGS.num_preprocessing_threads,
                               capacity=500)
        with tf.name_scope(FLAGS.dataset_name + '_prefetch_queue'):
            batch_queue = slim.prefetch_queue.prefetch_queue([b_image, b_pixel_cls_label, b_pixel_cls_weight,
                                                              b_pixel_link_label, b_pixel_link_weight], capacity=50)
    return batch_queue    
        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse=reuse):
            with tf.name_scope(config.clone_scopes[clone_idx]) as clone_scope:
                with tf.device(gpu) as clone_device:
                    ...
                    net.build_loss(pixel_cls_labels=b_pixel_cls_label, pixel_cls_weights=b_pixel_cls_weight,
                                   pixel_link_labels=b_pixel_link_label, pixel_link_weights=b_pixel_link_weight,
...
def train(train_op):
    ...
    slim.learning.train(train_op, logdir=FLAGS.train_dir, init_fn=init_fn, summary_op=summary_op,
                        number_of_steps=FLAGS.max_number_of_steps, log_every_n_steps=FLAGS.log_every_n_steps,
                        save_summaries_secs=30, saver=saver, save_interval_secs=1200, session_config=sess_config)
